...
export default function GovermentProfile() {
  ...
  const formik = useFormik({
    initialValues: {
      fullName: "Dr. Nimal Siriwardena",
      address: "Ministry of Agriculture, Battaramulla",
      ...
      <main className="flex-grow px-4 md:px-8 max-w-8xl mx-auto w-full pb-8 space-y-8">
        ...
        <div>
          ...
          <p className="text-lg text-muted-foreground mt">
            Manage your government administrative profile and credentials.
          </p>
        </div>
          ...
          <div className="md:col-span-8">
            <Card className="border-border shadow-sm">
              <CardHeader className="border-b border-border bg-primary/5 pb-4">
                ...
                <CardDescription>Update your contact information and government ID details.</CardDescription>
              </CardHeader>
              <CardContent className="pt-6 space-y-5">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5">Department / Address</label>
                  <div className="relative group">
                    <Input
                      type="text"
                ...
                <div className="pt-4 space-y-2">
                  <label className="block text-sm font-medium mb-1.5">National ID / Govt ID Upload</label>
                  <div className="flex items-center gap-4">
                    <Button type="button" variant="outline" className="relative cursor-pointer">
                      <Upload className="w-4 h-4 mr-2" />
                      {nicFileName ? "Change ID" : "Upload ID"}
                      <input 
                        type="file" 
                        accept="image/*,.pdf" 
                        className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" 
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Upload a clear photo or PDF of your National Identity Card or Government issued ID.</p>
                </div>
              </CardContent>
